...
const OrderForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // تمييز كل لون ومقاس برقم لربطهم بسهولة
    const colors = formData.colorSizePairs
      .map((pair, index) => `${pair.color} ${index + 1}`)
      .join("، ");

    const sizes = formData.colorSizePairs
      .map((pair, index) => `${pair.size} ${index + 1}`)
      .join("، ");

    const colorSizeDetails = formData.colorSizePairs
      .map(
        (pair, index) => `قطعة ${index + 1}: ${pair.color} - مقاس ${pair.size}`
      )
      .join(" | ");
    ...
    const submissionData = {
      ...
      color_size_relation: colorSizeDetails,
      order_details: colorSizeDetails,
    };
